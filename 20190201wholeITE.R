...
data_confounders_to_10y <- data_first_treatment_12ow_relevant[c(
  "outcome_4","AB_nose_infection","type_AB_nose", "outcome_weight", "ID", 
  "sex",
  "nr_medication",
  "COPD_morb",
...
uni10y<-glm(outcome_4~AB_nose_infection, weights = outcome_weight, data = subset(data_confounders_to, age<10), family = binomial)

summary(uni10y)

cbind( exp(coef(uni10y)), exp(summary(uni10y)$coefficients[,1] - 1.96*summary(uni10y)$coefficients[,2]), exp(summary(uni10y)$coefficients[,1] + 1.96*summary(uni10y)$coefficients[,2]) )
...
cbind( exp(coef(uni10ycov)), exp(summary(uni10ycov)$coefficients[,1] - 1.96*summary(uni10ycov)$coefficients[,2]), exp(summary(uni10ycov)$coefficients[,1] + 1.96*summary(uni10ycov)$coefficients[,2]) )
...
uni10y_type<-glm(outcome_4 ~ factor(type_AB_nose),weights = outcome_weight, data = subset(data_confounders_to, age<10), family = binomial)

summary(uni10y_type)

cbind( exp(coef(uni10y_type)), exp(summary(uni10y_type)$coefficients[,1] - 1.96*summary(uni10y_type)$coefficients[,2]), exp(summary(uni10y_type)$coefficients[,1] + 1.96*summary(uni10y_type)$coefficients[,2]) )
...
#Error in `contrasts<-`(`*tmp*`, value = contr.funs[1 + isOF[nn]]) : 
#?Ա?ֻ????????��????????��?????ε?????
summary(multi_type10y)

cbind( exp(coef(multi_type10y)), exp(summary(multi_type10y)$coefficients[,1] - 1.96*summary(multi_type10y)$coefficients[,2]), exp(summary(multi_type10y)$coefficients[,1] + 1.96*summary(multi_type10y)$coefficients[,2]) )
...
design.mnps10y <- svydesign(ids=~1, weights = ~ wt10y*outcome_weight, data = data_confounders_to_10y)
...
#Warning messages:
#  1: package 'bindrcpp' was built under R version 3.4.4 
...
for (i in 5:10){
  ...
  for (j in 1:10){
    ...
    p.cov<-subgroup.discovery::prim.cover(delta~., data = PRIM,
                                          ...
                                          plot= FALSE, 
                                          optimal.box= "2se")
    rule_cov<-p.cov$covers[[1]]$superrule      #how to deal with different covers here?
    #summary(p.cov)
...
#0.0267812
m_average<-mean(PRIM_np$delta,na.rm=TRUE)
#0.0267812
...
#Error in 1:best.box.idx : argument of length 0
...
for (i in 1:74){
  ...
  m_delta_m<- sum(subset(PRIM_np, box==1)$delta,na.rm=TRUE)/nrow(a)
  
  prim.table_np_mdelta$m_delta_m[i]<-  -m_delta_m
  
  betat <-nrow(a)/nrow(PRIM_np)
  prim.table_np_mdelta$betat[i]<- betat
  
  prim.table_np_mdelta$coverage[i]<-(m_average_MINUS - m_delta_m)*betat
...
set.seed(1234)
train_index <-sample(1:nrow(PRIM_np),nrow(PRIM_np)*0.66)
mytrain <- PRIM_np[train_index,]
mytest <- PRIM_np[-train_index,]
mean(mytrain$delta)
mean(mytest$delta)
nrow(mytrain)
nrow(mytest)
nrow(subset(mytrain,rule1==1))
nrow(subset(mytest,rule1==1))


mean(subset(mytrain,rule1==1)$delta)
mean(subset(mytrain,rule2==1)$delta)
mean(subset(mytrain,rule3==1)$delta)
mean(subset(mytrain,rule5==1)$delta)
mean(subset(mytest,rule1==1)$delta)
mean(subset(mytest,rule2==1)$delta)
mean(subset(mytest,rule3==1)$delta)
mean(subset(mytest,rule5==1)$delta)

### Provide descriptives of all variables of each rule
r1<- subset(mytrain,rule1==1)
r11<-dfSummary(r1)
write.table(r11, file="descriptives_rule1.csv", sep = ",")





# PRIM with delta coming from AB TYPE 0 VS 1 without postalcode
PRIM_np1 <- subset(data_first_treatment_12ow_relevant_0.4, select = c(age, sex, nr_medication,COPD_morb, migraine_morb, nr_contacts_resp, osteop_morb,poor_immune_response, alcmisb_morb,practice_size, Depressie_morb,aangaf_morb,hartfalen_morb,flu_epi,season, nr_contacts_infection, nr_prescriptions_AB, delta_1, ID))

prim.table_np1<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np1)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    beta<-j/100
    p.cov<-subgroup.discovery::prim.cover(delta_1~., data = PRIM_np1,
                                          peeling.quantile=alpha,
                                          min.support=beta, 
                                          plot= FALSE, 
                                          optimal.box= "2se")
    ...
    prim.table_np1[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
write.csv(prim.table_np1, file='prim.table_np1.csv')  

#Prevalence of the original population
m_average1<-mean(PRIM_np1$delta_1,na.rm=TRUE)
# 0.01661056
...
for (i in 1:56){
  ...
  myString <- paste("subset(PRIM_np1,", 
                    paste(gsub(',','&',prim.table_np1$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  
  a<-eval(parse(text=myString))  # now we tell R to look at the string as a command and execute it
  
  box<- ifelse(PRIM_np1$ID %in% a$ID,1,0) 
  
  # PRIM_np1[,ncol(PRIM_np1)+1]<-PRIM_np1$box
  # names(PRIM_np1)[ncol(PRIM_np1)]<-paste0("box_",i)
  
  m_delta_1<-sum(subset(PRIM_np1, box==1)$delta_1,na.rm=TRUE)/nrow(a)
  # m_delta_1<-sum(subset(PRIM_np1, box==1)$delta_1,na.rm=TRUE)/nrow(PRIM_np1)     confirm denominator!!!
  
  
  prim.table_np1$m_delta_1[i]<-m_delta_1
  
  betat <-nrow(a)/nrow(PRIM_np1)
  prim.table_np1$betat[i]<- betat
  
  # prim.table_np1$coverage1[i]<-(m_delta_1-m_average)*betat
  prim.table_np1$coverage2[i]<-(m_average1-m_delta_1)*betat
  
  write.csv( prim.table_np1, file='prim.table_np1_coverage.csv')
...
prim.table_np1_mdelta<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np1_mdelta)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    ...
    prim.table_np1_mdelta[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
#Error in 1:best.box.idx : argument of length 0

write.csv(prim.table_np1_mdelta, file='prim.table_np1_minus.csv')  


#Prevalence of the original population
m_average_MINUS1<- -mean(PRIM_np1$delta_1,na.rm=TRUE)
#-0.01661056
...
for (i in 1:48){
  ...
  myString <- paste("subset(PRIM_np1,", 
                    paste(gsub(',','&',prim.table_np1_mdelta$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  ...
  m_delta_m<- sum(subset(PRIM_np1, box==1)$delta_1,na.rm=TRUE)/nrow(a)
  
  prim.table_np1_mdelta$m_delta_m[i]<-  -m_delta_m
  
  betat <-nrow(a)/nrow(PRIM_np1)
  prim.table_np1_mdelta$betat[i]<- betat
  
  prim.table_np1_mdelta$coverage[i]<-(m_average_MINUS1 - m_delta_m)*betat
  
  
  write.csv( prim.table_np1_mdelta, file='prim.table_np1_coverage_m.csv')
...
set.seed(1234)
p.cov1<-subgroup.discovery::prim.cover(delta_1~. -ID, data = PRIM_np1,
                                      peeling.quantile=0.05,
                                      min.support=0.05, 
                                      plot= TRUE, 
                                      optimal.box= "best")
...
set.seed(1234)
p.cov1_m<-subgroup.discovery::prim.cover(-delta_1~. -ID, data = PRIM_np1,
                                        peeling.quantile=0.05,
                                        min.support=0.02, 
                                        plot= TRUE, 
                                        optimal.box= "best")
...
rule11<-p.cov1$covers[[1]]$superrule
rule12<-p.cov1$covers[[2]]$superrule
rule13<-p.cov1$covers[[3]]$superrule
rule14<-p.cov1$covers[[4]]$superrule

rule15<-p.cov1_m$covers[[1]]$superrule
rule16<-p.cov1_m$covers[[2]]$superrule
rule17<-p.cov1_m$covers[[3]]$superrule
rule18<-p.cov1_m$covers[[4]]$superrule


#interst box


myString11 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule11)), 
                         ")"
                   ))     # this will glue three strings together
myString12 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule12)), 
                         ")"
                   ))     # this will glue three strings together
myString13 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule13)), 
                         ")"
                   ))     # this will glue three strings together
myString14 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule14)), 
                         ")"
                   ))     # this will glue three strings together
myString15 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule15)), 
                         ")"
                   ))     # this will glue three strings together

myString16 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule16)), 
                         ")"
                   ))     # this will glue three strings together
myString17 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule17)), 
                         ")"
                   ))     # this will glue three strings together
myString18 <- paste("subset(PRIM_np1,", 
                   paste(gsub(',','&', toString(rule18)), 
                         ")"
                   ))     # this will glue three strings together

# a is the subset that follow the rule

a11<-eval(parse(text=myString11))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a12<-eval(parse(text=myString12))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a13<-eval(parse(text=myString13))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a14<-eval(parse(text=myString14))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a15<-eval(parse(text=myString15))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a16<-eval(parse(text=myString16))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a17<-eval(parse(text=myString17))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)
a18<-eval(parse(text=myString18))  # now we tell R to look at the string as a command and execute it
#risk_data1<-rbind(a,risk_data)



PRIM_np1$rule11<- as.factor(ifelse(PRIM_np1$ID %in% a11$ID,1,0) )
PRIM_np1$rule12<- as.factor(ifelse(PRIM_np1$ID %in% a12$ID,1,0) )
PRIM_np1$rule13<- as.factor(ifelse(PRIM_np1$ID %in% a13$ID,1,0) )
PRIM_np1$rule14<- as.factor(ifelse(PRIM_np1$ID %in% a14$ID,1,0) )
PRIM_np1$rule15<- as.factor(ifelse(PRIM_np1$ID %in% a15$ID,1,0) )
PRIM_np1$rule16<- as.factor(ifelse(PRIM_np1$ID %in% a16$ID,1,0) )
PRIM_np1$rule17<- as.factor(ifelse(PRIM_np1$ID %in% a17$ID,1,0) )
PRIM_np1$rule18<- as.factor(ifelse(PRIM_np1$ID %in% a18$ID,1,0) )
str(PRIM_np1)

# TAKE the box with different rules as variables to glm

#AIC to select significant variables
fit_rules1 <- glm( delta_1 ~., data  = select(PRIM_np1, -ID), family = gaussian)
# Select most predictive variables
AIC1 <- stepAIC(fit_rules1)
# FYI show formula of the final model
formula(AIC1)

summary(fit_rules1)
coefficients <-summary.glm(fit_rules1)$coefficients

write.csv( coefficients , file='fit_rules_AIC1.csv')

# PRIM with delta coming from AB TYPE 0 VS 2 without postalcode
PRIM_np2 <- subset(data_first_treatment_12ow_relevant_0.4, select = c(age, sex, nr_medication,COPD_morb, migraine_morb, nr_contacts_resp, osteop_morb,poor_immune_response, alcmisb_morb,practice_size, Depressie_morb,aangaf_morb,hartfalen_morb,flu_epi,season, nr_contacts_infection, nr_prescriptions_AB, delta_2, ID))

prim.table_np2<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np2)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    beta<-j/100
    p.cov<-subgroup.discovery::prim.cover(delta_2~., data = PRIM_np2,
                                          peeling.quantile=alpha,
                                          min.support=beta, 
                                          plot= FALSE, 
                                          optimal.box= "2se")
    ...
    prim.table_np2[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
write.csv(prim.table_np2, file='prim.table_np2.csv')  

#Prevalence of the original population
m_average2<-mean(PRIM_np2$delta_2,na.rm=TRUE)
#  0.02682467
...
for (i in 1:42){
  ...
  myString <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&',prim.table_np2$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  
  a<-eval(parse(text=myString))  # now we tell R to look at the string as a command and execute it
  
  box<- ifelse(PRIM_np2$ID %in% a$ID,1,0) 
  
  m_delta_2<-sum(subset(PRIM_np2, box==1)$delta_2,na.rm=TRUE)/nrow(a)
  
  prim.table_np2$m_delta_2[i]<-m_delta_2
  
  betat <-nrow(a)/nrow(PRIM_np2)
  prim.table_np2$betat[i]<- betat
  
  prim.table_np2$coverage[i]<-(m_average2-m_delta_2)*betat
  
  write.csv( prim.table_np2, file='prim.table_np2_coverage.csv')
...
prim.table_np2_mdelta<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np2_mdelta)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    ...
    prim.table_np2_mdelta[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
#Error in 1:best.box.idx : argument of length 0

write.csv(prim.table_np2_mdelta, file='prim.table_np2_minus.csv')  


#Prevalence of the original population
m_average_MINUS2<- -mean(PRIM_np2$delta_2,na.rm=TRUE)
#-0.02682467
...
for (i in 1:43){
  ...
  myString <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&',prim.table_np2_mdelta$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  ...
  m_delta_m<- sum(subset(PRIM_np2, box==1)$delta_2,na.rm=TRUE)/nrow(a)
  
  prim.table_np2_mdelta$m_delta_m[i]<-  -m_delta_m
  
  betat <-nrow(a)/nrow(PRIM_np2)
  prim.table_np2_mdelta$betat[i]<- betat
  
  prim.table_np2_mdelta$coverage[i]<-(m_average_MINUS2 - m_delta_m)*betat
  
  
  write.csv( prim.table_np2_mdelta, file='prim.table_np2_coverage_m.csv')
...
set.seed(1234)
p.cov2<-subgroup.discovery::prim.cover(delta_2~. -ID, data = PRIM_np2,
                                       peeling.quantile=0.05,
                                       min.support=0.01, 
                                       plot= TRUE, 
                                       optimal.box= "best")
...
set.seed(1234)
p.cov2_m<-subgroup.discovery::prim.cover(-delta_2~. -ID, data = PRIM_np2,
                                         peeling.quantile=0.05,
                                         min.support=0.03, 
                                         plot= TRUE, 
                                         optimal.box= "best")
...
rule21<-p.cov2$covers[[1]]$superrule
rule22<-p.cov2$covers[[2]]$superrule
rule23<-p.cov2$covers[[3]]$superrule
rule24<-p.cov2$covers[[4]]$superrule
rule25<-p.cov2$covers[[5]]$superrule
rule26<-p.cov2$covers[[6]]$superrule

rule27<-p.cov2_m$covers[[1]]$superrule
rule28<-p.cov2_m$covers[[2]]$superrule
rule29<-p.cov2_m$covers[[3]]$superrule
...
myString21 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule21)), 
                          ")"
                    ))     # this will glue three strings together
myString22 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule22)), 
                          ")"
                    ))     # this will glue three strings together
myString23 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule23)), 
                          ")"
                    ))     # this will glue three strings together
myString24 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule24)), 
                          ")"
                    ))     # this will glue three strings together
myString25 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule25)), 
                          ")"
                    ))     # this will glue three strings together

myString26 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule26)), 
                          ")"
                    ))     # this will glue three strings together
myString27 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule27)), 
                          ")"
                    ))     # this will glue three strings together
myString28 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule28)), 
                          ")"
                    ))     # this will glue three strings together
myString29 <- paste("subset(PRIM_np2,", 
                    paste(gsub(',','&', toString(rule29)), 
                          ")"
                    ))     # this will glue three strings together
# a is the subset that follow the rule

a21<-eval(parse(text=myString21))  # now we tell R to look at the string as a command and execute it

a22<-eval(parse(text=myString22))  # now we tell R to look at the string as a command and execute it

a23<-eval(parse(text=myString23))  # now we tell R to look at the string as a command and execute it

a24<-eval(parse(text=myString24))  # now we tell R to look at the string as a command and execute it

a25<-eval(parse(text=myString25))  # now we tell R to look at the string as a command and execute it

a26<-eval(parse(text=myString26))  # now we tell R to look at the string as a command and execute it

a27<-eval(parse(text=myString27))  # now we tell R to look at the string as a command and execute it

a28<-eval(parse(text=myString28))  # now we tell R to look at the string as a command and execute it

a29<-eval(parse(text=myString29))  # now we tell R to look at the string as a command and execute it


PRIM_np2$rule21<- as.factor(ifelse(PRIM_np2$ID %in% a21$ID,1,0) )
PRIM_np2$rule22<- as.factor(ifelse(PRIM_np2$ID %in% a22$ID,1,0) )
PRIM_np2$rule23<- as.factor(ifelse(PRIM_np2$ID %in% a23$ID,1,0) )
PRIM_np2$rule24<- as.factor(ifelse(PRIM_np2$ID %in% a24$ID,1,0) )
PRIM_np2$rule25<- as.factor(ifelse(PRIM_np2$ID %in% a25$ID,1,0) )
PRIM_np2$rule26<- as.factor(ifelse(PRIM_np2$ID %in% a26$ID,1,0) )
PRIM_np2$rule27<- as.factor(ifelse(PRIM_np2$ID %in% a27$ID,1,0) )
PRIM_np2$rule28<- as.factor(ifelse(PRIM_np2$ID %in% a28$ID,1,0) )
PRIM_np2$rule29<- as.factor(ifelse(PRIM_np2$ID %in% a29$ID,1,0) )
str(PRIM_np2)

# TAKE the box with different rules as variables to glm

#AIC to select significant variables
fit_rules2 <- glm( delta_2 ~., data  = select(PRIM_np2, -ID), family = gaussian)
# Select most predictive variables
AIC2 <- stepAIC(fit_rules2)
# FYI show formula of the final model
formula(AIC2)

summary(fit_rules2)
coefficients <-summary.glm(fit_rules2)$coefficients

write.csv( coefficients , file='fit_rules_AIC2.csv')


# PRIM with delta coming from AB TYPE 0 VS 3 without postalcode
PRIM_np3 <- subset(data_first_treatment_12ow_relevant_0.4, select = c(age, sex, nr_medication,COPD_morb, migraine_morb, nr_contacts_resp, osteop_morb,poor_immune_response, alcmisb_morb,practice_size, Depressie_morb,aangaf_morb,hartfalen_morb,flu_epi,season, nr_contacts_infection, nr_prescriptions_AB, delta_3, ID))

prim.table_np3<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np3)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    beta<-j/100
    p.cov<-subgroup.discovery::prim.cover(delta_3~., data = PRIM_np3,
                                          peeling.quantile=alpha,
                                          min.support=beta, 
                                          plot= FALSE, 
                                          optimal.box= "2se")
    ...
    prim.table_np3[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
write.csv(prim.table_np3, file='prim.table_np3.csv')  

#Prevalence of the original population
m_average3<-mean(PRIM_np3$delta_3,na.rm=TRUE)
#  0.05604153
...
for (i in 1:48){
  ...
  myString <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&',prim.table_np3$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  
  a<-eval(parse(text=myString))  # now we tell R to look at the string as a command and execute it
  
  box<- ifelse(PRIM_np3$ID %in% a$ID,1,0) 
  
  m_delta_3<-sum(subset(PRIM_np3, box==1)$delta_3,na.rm=TRUE)/nrow(a)
  
  prim.table_np3$m_delta_3[i]<-m_delta_3
  
  betat <-nrow(a)/nrow(PRIM_np3)
  prim.table_np3$betat[i]<- betat
  
  prim.table_np3$coverage[i]<-(m_average3-m_delta_3)*betat
  
  write.csv( prim.table_np3, file='prim.table_np3_coverage.csv')
...
prim.table_np3_mdelta<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np3_mdelta)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    ...
    prim.table_np3_mdelta[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
#Error in 1:best.box.idx : argument of length 0

write.csv(prim.table_np3_mdelta, file='prim.table_np3_minus.csv')  


#Prevalence of the original population
m_average_MINUS3<- -mean(PRIM_np3$delta_3,na.rm=TRUE)
#-0.05604153
...
for (i in 1:41){
  ...
  myString <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&',prim.table_np3_mdelta$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  ...
  m_delta_m<- sum(subset(PRIM_np3, box==1)$delta_3,na.rm=TRUE)/nrow(a)
  
  prim.table_np3_mdelta$m_delta_m[i]<-  -m_delta_m
  
  betat <-nrow(a)/nrow(PRIM_np3)
  prim.table_np3_mdelta$betat[i]<- betat
  
  prim.table_np3_mdelta$coverage[i]<-(m_average_MINUS3 - m_delta_m)*betat
  
  
  write.csv( prim.table_np3_mdelta, file='prim.table_np3_coverage_m.csv')
...
set.seed(1234)
p.cov3<-subgroup.discovery::prim.cover(delta_3~. -ID, data = PRIM_np3,
                                       peeling.quantile=0.05,
                                       min.support=0.06, 
                                       plot= TRUE, 
                                       optimal.box= "best")
...
set.seed(1234)
p.cov3_m<-subgroup.discovery::prim.cover(-delta_3~. -ID, data = PRIM_np3,
                                         peeling.quantile=0.05,
                                         min.support=0.01, 
                                         plot= TRUE, 
                                         optimal.box= "best")
...
rule31<-p.cov3$covers[[1]]$superrule
rule32<-p.cov3$covers[[2]]$superrule
rule33<-p.cov3$covers[[3]]$superrule
rule34<-p.cov3$covers[[4]]$superrule
rule35<-p.cov3$covers[[5]]$superrule

rule36<-p.cov3_m$covers[[1]]$superrule
rule37<-p.cov3_m$covers[[2]]$superrule
rule38<-p.cov3_m$covers[[3]]$superrule
rule39<-p.cov3_m$covers[[4]]$superrule
rule310<-p.cov3_m$covers[[5]]$superrule
rule311<-p.cov3_m$covers[[6]]$superrule
rule312<-p.cov3_m$covers[[7]]$superrule


#interst box


myString31 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule31)), 
                          ")"
                    ))     # this will glue three strings together
myString32 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule32)), 
                          ")"
                    ))     # this will glue three strings together
myString33 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule33)), 
                          ")"
                    ))     # this will glue three strings together
myString34 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule34)), 
                          ")"
                    ))     # this will glue three strings together
myString35 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule35)), 
                          ")"
                    ))     # this will glue three strings together

myString36 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule36)), 
                          ")"
                    ))     # this will glue three strings together
myString37 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule37)), 
                          ")"
                    ))     # this will glue three strings together
myString38 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule38)), 
                          ")"
                    ))     # this will glue three strings together
myString39 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule39)), 
                          ")"
                    ))     # this will glue three strings together
myString310 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule310)), 
                          ")"
                    ))     # this will glue three strings together
myString311 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule311)), 
                          ")"
                    ))     # this will glue three strings together
myString312 <- paste("subset(PRIM_np3,", 
                    paste(gsub(',','&', toString(rule312)), 
                          ")"
                    ))     # this will glue three strings together
...
a31<-eval(parse(text=myString31))  # now we tell R to look at the string as a command and execute it

a32<-eval(parse(text=myString32))  # now we tell R to look at the string as a command and execute it

a33<-eval(parse(text=myString33))  # now we tell R to look at the string as a command and execute it

a34<-eval(parse(text=myString34))  # now we tell R to look at the string as a command and execute it

a35<-eval(parse(text=myString35))  # now we tell R to look at the string as a command and execute it

a36<-eval(parse(text=myString36))  # now we tell R to look at the string as a command and execute it

a37<-eval(parse(text=myString37))  # now we tell R to look at the string as a command and execute it

a38<-eval(parse(text=myString38))  # now we tell R to look at the string as a command and execute it

a39<-eval(parse(text=myString39))  # now we tell R to look at the string as a command and execute it
a310<-eval(parse(text=myString310))  # now we tell R to look at the string as a command and execute it

a311<-eval(parse(text=myString311))  # now we tell R to look at the string as a command and execute it

a312<-eval(parse(text=myString312))  # now we tell R to look at the string as a command and execute it


PRIM_np3$rule31<- as.factor(ifelse(PRIM_np3$ID %in% a31$ID,1,0) )
PRIM_np3$rule32<- as.factor(ifelse(PRIM_np3$ID %in% a32$ID,1,0) )
PRIM_np3$rule33<- as.factor(ifelse(PRIM_np3$ID %in% a33$ID,1,0) )
PRIM_np3$rule34<- as.factor(ifelse(PRIM_np3$ID %in% a34$ID,1,0) )
PRIM_np3$rule35<- as.factor(ifelse(PRIM_np3$ID %in% a35$ID,1,0) )
PRIM_np3$rule36<- as.factor(ifelse(PRIM_np3$ID %in% a36$ID,1,0) )
PRIM_np3$rule37<- as.factor(ifelse(PRIM_np3$ID %in% a37$ID,1,0) )
PRIM_np3$rule38<- as.factor(ifelse(PRIM_np3$ID %in% a38$ID,1,0) )
PRIM_np3$rule39<- as.factor(ifelse(PRIM_np3$ID %in% a39$ID,1,0) )
PRIM_np3$rule310<- as.factor(ifelse(PRIM_np3$ID %in% a310$ID,1,0) )
PRIM_np3$rule311<- as.factor(ifelse(PRIM_np3$ID %in% a311$ID,1,0) )
PRIM_np3$rule312<- as.factor(ifelse(PRIM_np3$ID %in% a312$ID,1,0) )
str(PRIM_np3)

# TAKE the box with different rules as variables to glm

#AIC to select significant variables
fit_rules3 <- glm( delta_3 ~., data  = select(PRIM_np3, -ID), family = gaussian)
# Select most predictive variables
AIC3 <- stepAIC(fit_rules3)
# FYI show formula of the final model
formula(AIC3)

summary(fit_rules3)
coefficients <-summary.glm(fit_rules3)$coefficients

write.csv( coefficients , file='fit_rules_AIC3.csv')


# PRIM with delta coming from AB TYPE 0 VS 4 without postalcode
PRIM_np4 <- subset(data_first_treatment_12ow_relevant_0.4, select = c(age, sex, nr_medication,COPD_morb, migraine_morb, nr_contacts_resp, osteop_morb,poor_immune_response, alcmisb_morb,practice_size, Depressie_morb,aangaf_morb,hartfalen_morb,flu_epi,season, nr_contacts_infection, nr_prescriptions_AB, delta_4, ID))

prim.table_np4<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np4)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    beta<-j/100
    p.cov<-subgroup.discovery::prim.cover(delta_4~., data = PRIM_np4,
                                          peeling.quantile=alpha,
                                          min.support=beta, 
                                          plot= FALSE, 
                                          optimal.box= "2se")
    ...
    prim.table_np4[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
write.csv(prim.table_np4, file='prim.table_np4.csv')  

#Prevalence of the original population
m_average4<-mean(PRIM_np4$delta_4,na.rm=TRUE)
#  0.02068188
...
for (i in 1:65){
  ...
  myString <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&',prim.table_np4$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  
  a<-eval(parse(text=myString))  # now we tell R to look at the string as a command and execute it
  
  box<- ifelse(PRIM_np4$ID %in% a$ID,1,0) 
  
  m_delta_4<-sum(subset(PRIM_np4, box==1)$delta_4,na.rm=TRUE)/nrow(a)
  
  prim.table_np4$m_delta_4[i]<-m_delta_4
  
  betat <-nrow(a)/nrow(PRIM_np4)
  prim.table_np4$betat[i]<- betat
  
  prim.table_np4$coverage[i]<-(m_average4-m_delta_4)*betat
  
  write.csv( prim.table_np4, file='prim.table_np4_coverage.csv')
...
prim.table_np4_mdelta<-data.frame(matrix(ncol=(6)))
colnames(prim.table_np4_mdelta)<-c("alpha","beta","number_cov","rule_cov","number_div","rule_div")
...
for (i in 5:10){
  ...
  for (j in 1:10){
    ...
    prim.table_np4_mdelta[10*i+j-10,]<-c(alpha,beta,length(rule_cov),toString(rule_cov),length(rule_div),toString(rule_div))
    
  }
}
#Error in 1:best.box.idx : argument of length 0

write.csv(prim.table_np4_mdelta, file='prim.table_np4_minus.csv')  


#Prevalence of the original population
m_average_MINUS4<- -mean(PRIM_np4$delta_4,na.rm=TRUE)
#-0.02068188
...
for (i in 1:71){
  ...
  myString <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&',prim.table_np4_mdelta$rule_cov[i]), 
                          ")"
                    ))# this will glue three strings together
  ...
  m_delta_m<- sum(subset(PRIM_np4, box==1)$delta_4,na.rm=TRUE)/nrow(a)
  
  prim.table_np4_mdelta$m_delta_m[i]<-  -m_delta_m
  
  betat <-nrow(a)/nrow(PRIM_np4)
  prim.table_np4_mdelta$betat[i]<- betat
  
  prim.table_np4_mdelta$coverage[i]<-(m_average_MINUS4 - m_delta_m)*betat
  
  
  write.csv( prim.table_np4_mdelta, file='prim.table_np4_coverage_m.csv')
...
set.seed(1234)
p.cov4<-subgroup.discovery::prim.cover(delta_4~. -ID, data = PRIM_np4,
                                       peeling.quantile=0.05,
                                       min.support=0.08, 
                                       plot= TRUE, 
                                       optimal.box= "best")
...
set.seed(1234)
p.cov4_m<-subgroup.discovery::prim.cover(-delta_4~. -ID, data = PRIM_np4,
                                         peeling.quantile=0.05,
                                         min.support=0.02, 
                                         plot= TRUE, 
                                         optimal.box= "best")
...
rule41<-p.cov4$covers[[1]]$superrule
rule42<-p.cov4$covers[[2]]$superrule
rule43<-p.cov4$covers[[3]]$superrule

rule44<-p.cov4_m$covers[[1]]$superrule
rule45<-p.cov4_m$covers[[2]]$superrule
rule46<-p.cov4_m$covers[[3]]$superrule
rule47<-p.cov4_m$covers[[4]]$superrule


#interst box


myString41 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule41)), 
                          ")"
                    ))     # this will glue three strings together
myString42 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule42)), 
                          ")"
                    ))     # this will glue three strings together
myString43 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule43)), 
                          ")"
                    ))     # this will glue three strings together
myString44 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule44)), 
                          ")"
                    ))     # this will glue three strings together
myString45 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule45)), 
                          ")"
                    ))     # this will glue three strings together

myString46 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule46)), 
                          ")"
                    ))     # this will glue three strings together
myString47 <- paste("subset(PRIM_np4,", 
                    paste(gsub(',','&', toString(rule47)), 
                          ")"
                    ))     # this will glue three strings together
...
a41<-eval(parse(text=myString41))  # now we tell R to look at the string as a command and execute it

a42<-eval(parse(text=myString42))  # now we tell R to look at the string as a command and execute it

a43<-eval(parse(text=myString43))  # now we tell R to look at the string as a command and execute it

a44<-eval(parse(text=myString44))  # now we tell R to look at the string as a command and execute it

a45<-eval(parse(text=myString45))  # now we tell R to look at the string as a command and execute it

a46<-eval(parse(text=myString46))  # now we tell R to look at the string as a command and execute it

a47<-eval(parse(text=myString47))  # now we tell R to look at the string as a command and execute it



PRIM_np4$rule41<- as.factor(ifelse(PRIM_np4$ID %in% a41$ID,1,0) )
PRIM_np4$rule42<- as.factor(ifelse(PRIM_np4$ID %in% a42$ID,1,0) )
PRIM_np4$rule43<- as.factor(ifelse(PRIM_np4$ID %in% a43$ID,1,0) )
PRIM_np4$rule44<- as.factor(ifelse(PRIM_np4$ID %in% a44$ID,1,0) )
PRIM_np4$rule45<- as.factor(ifelse(PRIM_np4$ID %in% a45$ID,1,0) )
PRIM_np4$rule46<- as.factor(ifelse(PRIM_np4$ID %in% a46$ID,1,0) )
PRIM_np4$rule47<- as.factor(ifelse(PRIM_np4$ID %in% a47$ID,1,0) )
...
#AIC to select significant variables
fit_rules4 <- glm( delta_4 ~., data  = select(PRIM_np4, -ID), family = gaussian)
# Select most predictive variables
AIC4 <- stepAIC(fit_rules4)
# FYI show formula of the final model
formula(AIC4)

summary(fit_rules4)
coefficients <-summary.glm(fit_rules4)$coefficients

write.csv( coefficients , file='fit_rules_AIC4.csv')
