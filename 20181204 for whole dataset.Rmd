...
onlynose[cols.num] <- sapply(onlynose[cols.num],as.numeric)
onlynose$date <- dmy(onlynose$date)
...
z1<-dfSummary(anti_fn)
write.table(z1, file="descriptives_raw_nose_anti.csv", sep = ",")

z2<-dfSummary(semi_fn)
write.table(z2, file="descriptives_raw_nose_semi.csv", sep = ",")

###date

hist(onlynose$ days_prev_cont)
nrow(subset(onlynose,outcome_2==1 & outcome_4==0))/nrow(subset(onlynose,outcome_4==0))

mean(month(onlynose$date)==12)
mean(month(onlynose$date)<4)

onlynose_12ow <- mutate(onlynose,outcome_weight=ifelse(month(date)==12 & is.na(outcome_4),
                                                          (31-day(onlynose$date))/28, 1))
...
mi<-summary(data_confounders_mi)
write.table(mi, file="descriptives_c_mi.csv", sep = ",")

write.mice.imputation(mi.res=data_confounders_mi, name="data_confounders_mii")
#Error in `[<-.data.frame`(`*tmp*`, , "Imputation_", value = c(NA_real_,  : 
#  replacement has 121960 rows, data has 146352


data_confounders_mi_to <-merge(data_confounders_mii,data_confounders_to, by="ID")
...
model_fit<-with(data_confounders_mi, glm(outcome_4~AB_nose_infection,family = binomial))
...
design.mnps <- svydesign(ids=~1, weights=~w*outcome_weight, data=data_confounders_to) 
## survey
# model1
