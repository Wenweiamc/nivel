...
data_first_treatment_12ow <- mutate(data_first_treatment, 
outcome4_weighted=ifelse(is.na(outcome_4),outcome_weight, outcome_4))
...
outcome_weight=ifelse(month(date)==12 & is.na(outcome_4),(31-day(data_first_treatment$date))/28, 1),
outcome4_weighted=ifelse(is.na(outcome_4),outcome_weight, as.numeric(outcome_4)))
...
data_confounders_to10 <- mutate(subset(data_confounders_to, age>=10), 
w10=get.weights(PS_Model10y,stop.method = "es.mean" ))
...
mnp.balance18
write.table(mnp.balance18, file="P:/NZR/NZR-001/Innovatieve Onderzoeksmethoden/2. Machine learning antibiotica bij ontstekingen/03. Dataverwerking/6. output R/20190102wqi/mnp.balance_type18.csv", sep = ",")
...
#Multivariate Imputation by Chained Equations
confounders_mi<-mice(data_confounders, m=5,seed = 1234)
confounders_mi18<-mice(subset(data_confounders, age>=18), m=5,seed = 1234)

densityplot(confounders_mi)
confounders_mi$loggedEvents
confounders_mi18$loggedEvents

pred <- make.predictorMatrix(data_confounders, age>=18)
pred18 <- make.predictorMatrix(subset(data_confounders, age>=18))

pred18[c("nr_contacts_resp", "postalcode"), c("nr_contacts_resp", "postalcode")] <- 0
pred18[c("nr_prescriptions_AB", "postalcode"), c("nr_prescriptions_AB", "postalcode")] <- 0
pred18[c("nr_contacts_infection", "postalcode"), c("nr_contacts_infection", "postalcode")] <- 0

pred18
mi<-mice(subset(data_confounders, age>=18),pred=pred18, m=5,seed = 1234)
class(mi)

data_confounders_mi18<-mi$data
class(data_confounders_mi18)


data_confounders_mi_to18 <-merge(data_confounders_mi18,subset(data_confounders_to, age>=18)[,c("outcome_4","AB_nose_infection","type_AB_nose", "outcome_weight", "ID")], by="ID")

data_confounders_mi_to18 <- mutate(data_confounders_mi_to18, 
wt18=get.weights(MNPS_cto_type18,stop.method = "es.mean" ))

#design.mnps18_mi<-with(mi, svydesign(ids=~1, weights = ~ wt18*outcome_weight))

design.mnps18_mi <- svydesign(ids=~1, weights = ~ wt18*outcome_weight, data = data_confounders_mi_to18)
  
#design.mnps18_mi_pooled<-pool(design.mnps18_mi)
#summary(design.mnps18_mi_pooled)
...
# model 2
glm_type18_part <- svyglm(outcome_4 ~ type_AB_nose + season + postalcode + hartfalen_morb + nr_prescriptions_AB + nr_contacts_infection + poor_immune_response + nr_medication + COPD_morb + migraine_morb + sex, design = design.mnps18, family=quasibinomial)
...
glm_type18_part_mi <- svyglm(outcome_4 ~ type_AB_nose + season + postalcode + hartfalen_morb + nr_prescriptions_AB + nr_contacts_infection + poor_immune_response + nr_medication + COPD_morb + migraine_morb + sex, design = design.mnps18_mi, family=quasibinomial)

summary(glm_type18_part_mi)

cbind( exp(coef(glm_type18_part_mi)), exp(summary(glm_type18_part_mi)$coefficients[,1] - 1.96*summary(glm_type18_part_mi)$coefficients[,2]), exp(summary(glm_type18_part_mi)$coefficients[,1] + 1.96*summary(glm_type18_part_mi)$coefficients[,2]) )


# model 2.2 no postalcode
glm_type18_part_np <- svyglm(outcome_4 ~ type_AB_nose + season + hartfalen_morb + nr_prescriptions_AB + nr_contacts_infection + poor_immune_response + nr_medication + COPD_morb + migraine_morb + sex, design = design.mnps18, family=quasibinomial)
...
glm_type18_part_mi_np <- svyglm(outcome_4 ~ type_AB_nose + season + hartfalen_morb + nr_prescriptions_AB + nr_contacts_infection + poor_immune_response + nr_medication + COPD_morb + migraine_morb + sex, design = design.mnps18_mi, family=quasibinomial)

summary(glm_type18_part_mi_np)

cbind( exp(coef(glm_type18_part_mi_np)), exp(summary(glm_type18_part_mi_np)$coefficients[,1] - 1.96*summary(glm_type18_part_mi_np)$coefficients[,2]), exp(summary(glm_type18_part_mi_np)$coefficients[,1] + 1.96*summary(glm_type18_part_mi_np)$coefficients[,2]) )
...
#mnp.balance7
write.table(mnp.balance7, file="P:/NZR/NZR-001/Innovatieve Onderzoeksmethoden/2. Machine learning antibiotica bij ontstekingen/03. Dataverwerking/6. output R/20190102wqi/mnp.balance_type7.csv", sep = ",")
...
# model 2
glm_type7_part <- svyglm(outcome_4 ~ type_AB_nose + season +hartfalen_morb + postalcode, design = design.mnps7, family=quasibinomial)
...
# model 2.1 no postalcode
glm_type7_part_np <- svyglm(outcome_4 ~ type_AB_nose + season +hartfalen_morb, design = design.mnps7, family=quasibinomial)
...
###########################
setwd("P:/NZR/NZR-001/Innovatieve Onderzoeksmethoden/2. Machine learning antibiotica bij ontstekingen/03. Dataverwerking/6. output R/20190102wqi/")
...
# a is the subset of dagang that follow the rule
